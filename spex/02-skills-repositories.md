...
With `skilled` You can use multiple skills repositories on a single device. Repositories are downloaded to `~/.skilled/repos` directory and symlinked to `~/.agents/skills` as needed. When skills repository is downloaded into `~/.skilled/repos` directory, we say it is **installed**. 
